...
def train(train_loader, validation_loader, optimizer, epoch, lr=1e-3):
    ...
    pbar = tqdm(range(epoch), desc="Training SimpleCNN")
    ...
    plot(title="SimpleCNN Train", sub_titles=["Loss", "IOU"], \
...
if __name__ == "__main__":
    model = SimpleCNN().to(device)
    model.load_state_dict(torch.load("checkpoints/SimpleCNN.pth"))   
    ...
    cv2.imwrite("SimpleCNN/example.jpg", image)
    cv2.waitKey(0)