...
def train(train_loader, validation_loader, optimizer, epoch, lr=1e-3):
    ...
    pbar = tqdm(range(epoch), desc="Training LeNet")
    ...
    plot(title="LeNet Train", sub_titles=["Loss", "IOU"], \
...
if __name__ == "__main__":
    model = LeNet().to(device)
    model.load_state_dict(torch.load("checkpoints/LeNet.pth"))   
    ...
    cv2.imwrite("LeNet/example.jpg", image)
    cv2.waitKey(0)