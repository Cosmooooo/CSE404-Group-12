...
def train_epoch(model, optimizer, dataloader, e):
    ...
    epoch_cls_loss, epoch_reg_loss, epoch_iou = 0, 0, 0
    ...
    for images, bbox_labels, rois, rois_cls, rois_transform in tqdm(dataloader, desc=f"Training Epoch {e}", leave=False):
        ...
        cls_loss = model.cls_loss((classes_pred, bboxes_pred), (rois_cls, rois_transform))
        cls_loss.backward()
        optimizer.step()
        ...
        optimizer.zero_grad()
        classes_pred, bboxes_pred = model(images, rois)
        reg_loss = model.reg_loss((classes_pred, bboxes_pred), (rois_cls, rois_transform))
        reg_loss.backward()
        optimizer.step()
    ...
    return epoch_cls_loss / len(dataloader),\
...
def train(train_loader, validation_loader, optimizer, epoch, lr=1e-3):
    model = RCNN(2).to(device)
    optimizer = optimizer(model.parameters(), lr=lr)

    train_cls_losses, train_reg_losses, train_ious = [], [], []
    val_cls_losses, val_reg_losses, val_ious = [], [], []

    pbar = tqdm(range(epoch), desc="Training RCNN")
    for e in range(epoch):
        val_cls_loss, val_reg_loss, val_iou = test_epoch(model, validation_loader, e)
        train_cls_loss, train_reg_loss, train_iou = train_epoch(model, optimizer, train_loader, e)
        ...
        pbar.write(f"Train Epoch: {e}, classifier loss: {train_cls_loss:.5f} regressor loss: {train_reg_loss:.5f} IOU: {train_iou:.5f}")
        pbar.write(f"Validation Epoch: {e}, classifier loss: {val_cls_loss:.5f} regressor loss: {val_reg_loss:.5f} IOU: {val_iou:.5f}")
        pbar.update(1)
    
    plot(title="RCNN Train", sub_titles=["Classifier Loss", "Regressor Loss", "IOU"], \
         train_result=[train_cls_losses, train_reg_losses, train_ious], validate_result=[val_cls_losses, val_reg_losses, val_ious])
    
    return model

def test(model, test_loader):
    test_cls_loss, test_reg_loss, test_iou = test_epoch(model, test_loader, "-")
    print(f"Test Cls Loss: {test_cls_loss}, Test Reg Loss: {test_reg_loss} Test IOU: {test_iou}")
...
if __name__ == "__main__":
    model = RCNN(2).to(device)
    model.load_state_dict(torch.load("checkpoints/RCNN.pth"))   
    ...
    cv2.imwrite("RCNN/example.jpg", image)
    cv2.waitKey(0)
